...
void open_directory(char* directory, char* program, char* log, int time, int number) {
	DIR *dir;
	...
	{
 		sleep(time);
		rotation(directory, log, program, number, 0);
...
int main(int argc, char const *argv[]) {
	...
	open_directory(argv[1], argv[0], argv[2], time, number);
